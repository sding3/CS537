...
readi(struct inode *ip, char *dst, uint off, uint n)
{
    ...
    if (ip->type == T_SMALLFILE)
    { 
        //cprintf("readi, n is %d\n",n);
        // handle T_SMALLFILE
        int ibype;
        int maxBytes = (NDIRECT+1)*4;
        char *src = ((void*)ip->addrs)+off;

        for (ibype = 0; ibype < min(n,maxBytes); ibype++)
        {
            //cprintf("reading %d and putting to %p\n",(int) *src,dst);
            memmove(dst, src,1);
            dst++;
            src++;
        }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
    ...
    // start here
    if (off > ip->size || off + n < off){
        //cprintf("writei - out of bound\n");
        return -1;
    }

    if (off + n > MAXFILE * BSIZE && ip->type != T_SMALLFILE)
        n = MAXFILE * BSIZE - off;

    //cprintf("in writei\n");
    if (ip->type == T_SMALLFILE)
    {
        //cprintf("in writei , off = %d\n",off);
        // handle T_SMALLFILE
        int ibype;
        int maxBytes = (NDIRECT+1)*4;
        void *des = (void*)ip->addrs + off;
        //cprintf("offset = %d\n",off);

        for (ibype = 0; ibype < min(n,maxBytes-off); ibype++)
        {
            //ip->addrs[ibype] = src[ibype];
            //cprintf("writing %d to %p\n",(int) *src,des);
            memmove(des, src,1);
            des++;
            src++;
        }
        n = ibype;
        //cprintf("DEBUG: wrote %d bytes\n", n);
    }
    else
    {
        // handle T_FILE
    ...
    if (n > 0 && off > ip->size) // if n is postve AND offset > filesize
    {
        ip->size = off;
        iupdate(ip);
    }

    // make sure for SMALLFILE writes
    // update inode
    if (ip->type == T_SMALLFILE  )
    {
        
            ip->size = min(n+off,52);
            //cprintf("Setting the file size to %d\n", ip->size);
            iupdate(ip);
